# shellcheck shell=bash

[[ -n ${_ATLAS_ARGOCD_RENDER_LOADED:-} ]] && return 0
readonly _ATLAS_ARGOCD_RENDER_LOADED=1

argocd::render_root() {
  local template content root_name repo_url revision root_path
  template="${ATLAS_ROOT_DIR}/bootstrap/argocd/root-app.yaml.tpl"
  content=$(< "$template") || {
    runtime::die "unable to read Root template: ${template}"
    return 1
  }
  root_name=$(config::get ATLAS_ROOT_NAME) || return 1
  repo_url=$(config::get ATLAS_GIT_REPO_URL) || return 1
  revision=$(config::get ATLAS_GIT_REVISION) || return 1
  root_path=$(config::get ATLAS_ROOT_PATH) || return 1
  content=${content//@@ROOT_NAME@@/$root_name}
  content=${content//@@REPO_URL@@/$repo_url}
  content=${content//@@REVISION@@/$revision}
  content=${content//@@ROOT_PATH@@/$root_path}
  [[ $content != *@@* ]] || {
    runtime::die "Root template contains unresolved tokens"
    return 1
  }
  printf '%s\n' "$content" || return 1
}

argocd::render_seed() {
  local chart chart_file values
  chart_file=$(config::version ARGOCD_CHART_FILE) || return 1
  chart="${ATLAS_ROOT_DIR}/${chart_file}"
  values="${ATLAS_ROOT_DIR}/gitops/platform/management/argocd-self/values.yaml"

  helm template atlas-argocd "$chart" \
    --namespace argocd \
    --include-crds \
    --values "$values" || return 1
}

argocd::_validate_rendered_seed() {
  local manifest=$1
  grep -Fq 'kind: CustomResourceDefinition' "$manifest" || return 1
  grep -Fq 'name: applications.argoproj.io' "$manifest" || return 1
  grep -Fq 'name: atlas-argocd-server' "$manifest" || return 1
  grep -Fq "$(config::version ARGOCD_IMAGE)" "$manifest" || return 1
  grep -Fq "$(config::version REDIS_IMAGE)" "$manifest" || return 1
  ! grep -Eq '^kind: (Application|AppProject)$' "$manifest" || return 1
}

argocd::render() {
  runtime::phase render
  local chart chart_file expected_sha actual_sha output_dir
  chart_file=$(config::version ARGOCD_CHART_FILE) || return 1
  chart="${ATLAS_ROOT_DIR}/${chart_file}"
  expected_sha=$(config::version ARGOCD_CHART_SHA256) || return 1
  [[ -s $chart && ! -L $chart ]] || {
    runtime::die "locked Argo CD chart is missing: ${chart}"
    return 1
  }
  actual_sha=$(runtime::sha256 "$chart") || return 1
  [[ $actual_sha == "$expected_sha" ]] || {
    runtime::die "Argo CD chart checksum mismatch"
    return 1
  }

  output_dir="${ATLAS_STATE_DIR}/rendered"
  lock::assert_held || return 1
  runtime::atomic_capture "${output_dir}/argocd-seed.yaml" argocd::render_seed || return 1
  lock::assert_held || return 1
  runtime::atomic_capture "${output_dir}/root-app.yaml" argocd::render_root || return 1
  lock::assert_held || return 1
  argocd::_validate_rendered_seed "${output_dir}/argocd-seed.yaml" || {
    runtime::die "rendered Argo CD seed violates its contract"
    return 1
  }
  runtime::ok "rendered Bootstrap manifests: ${output_dir}"
}
